...
extension RenderTree {
    static func render(hstack: HStack, requestedSize: SizeInformation, properties: ViewProperties, builder: GuiViewBuilder) {
        if let group = hstack.content as? Group {
            let spacerCount = group.children.count(where: { $0 is Spacer })
            let sizeTaken = group.children.reduce(Float(0.0), { x, child in
                if !(child is Spacer) {
            ...
            let _ = group.children.reduce(Float(0.0), { x,child in
                if child is Spacer {
